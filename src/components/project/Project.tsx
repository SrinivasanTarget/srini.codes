  ...
  React.useEffect(() => {
    async function init() {
      try {
        ...
        if (!response.ok) {
          console.error(
            `Failed to fetch GitHub data for ${project.title} (${project.source}): ${response.status} ${response.statusText}`,
          )
        ...
        console.error(
          `Exception while fetching GitHub repository ${project.title} (${project.source}):`,
          error,
        )
    ...
    if (project.source.includes('github.com')) {
      // Only fetch if it's a github link
      init()
    }
          ...
          >
            ...
            {project.source.includes('github.com') && ( // Conditionally render stats
              <div className='px-1 pt-2 flex flex-wrap justify-center items-center text-xs text-custom-gray-medium'>
                ...
                {language !== 'N/A' && (
                  <div className='mb-1 px-2 py-1 rounded-md bg-custom-gray-dark'>{language}</div>
                )}
              </div>
            )}
          </a>
